...
2026-09-21 장비 작업 패킷: [무기·갑옷 6종](equipment.md)의 구매/장착과 실효 능력치를
도입한다. 기본 능력과 장비 차이를 분리해 성장·재장착·공개 재투영·저장복원 시 중복 합산을
막고, 전투 명중/피해/방어 및 모든 현재 은신 경로에 같은 계산을 적용한다.
Astra 백엔드 writer → 루트 장비 비교 UI·실제 구매/원정 전투/복원 브라우저 통합 →
Sol 독립 검토와 전체 검사 순서다. 전체 장비·전리품·클래스/주문 구현과는 구분한다.

2026-09-21 파벌 사건 작업 패킷: 원정 보고를 계기로 공개 파벌 상태와 수송/정산 두 사건을
예약한다. 원정의 구조 결과와 플레이어 지원·미개입에 따라 수송·물자·신뢰·시장 가격이 달라지고,
기한을 넘긴 휴식도 사건을 순서대로 한 번씩 진행시킨다. [세계 사건 계약](world-events.md)을 따른다.
Astra 순수 규칙 writer → 루트 transaction·NPC 기억·공개 context·UI/브라우저 통합 →
Sol 읽기 전용 검토와 전체 검사 순서다. 고정 템플릿 구현과 실제 Luna Planner·장기 품질은 구분한다.

2026-09-21 복수 조우 작업 패킷: 여관 이후 망루 매복 전투를 선택할 수 있게 한다.
전투 승리는 기존 원정의 접근 단계로 합류하며 단서 조사와 최종 구조 선택을 대신하지 않는다.
후퇴·패배 치료 뒤에는 잠입/계단 보강 경로가 남는다. 조우별 종료 기록과 1회 경험치,
레거시 저장 호환·전투 중 탐험 금지·두 번째 조우의 시간 누적·대상 ID 분리를 검증한다.
Astra 백엔드 writer → 루트 동적 전장 표시·브라우저 모험 통합 → Sol 읽기 전용 검토 →
전체 검사 순서다. 동일 격자를 재사용하는 두 조우이며 범용 전장 생성이나 장시간 플레이
완료를 뜻하지 않는다.

2026-09-21 NPC 기억 작업 패킷: 장부의 앞12개만 보내던 선택을 질문 관련도·최신성·시나리오
기억의 결정적 검색으로 교체한다. 대상 NPC의 공유 가능한 장부만 읽고 모델 입력 수량을 제한한다.
플레이어에게 들은 망루 보고는 직접 확인한 사실과 구분하며 기존 저장의 해당 기록도 정규화한다.
독립 기억 writer → 루트 narrator 질의/보고 출처 통합 → 실제 요청 payload·저장 회귀 →
읽기 전용 검토·전체 검사 순서다. 이 단계는 어휘 기반 검색이며 임베딩·LLM 요약·실모델의
2시간 의미적 일관성 검증을 완료한 것으로 취급하지 않는다.

2026-09-21 전투 v3 작업 패킷: 이동마다 적이 대응하던 규칙을 명시적인 플레이어 턴으로 바꾼다.
이동3칸·주요 행동1회·물약용 보조 행동1회, 턴 종료 시 적 대응으로 나누며 사용한 예산을 저장한다.
기존 단일/복수 적·BFS·주사위·사망 중단 검사를 유지한 채 새 대응 시점으로 전환하고,
예산 초과 거부·물약 소비·저장복원·재송신·세계 시간·공개 context·브라우저 조작을 검증한다.
독립 엔진 writer → 루트 UI/계약 통합 → 읽기 전용 독립 검토 → 전체 검사 순서다.
...
| P4a | AI-001 / P3 + 실제 접속 사양 | `backend/app/ai.py`, `ai_metrics.py`, `evaluation.py`, `scripts/evaluate_ai.py` | adapter·smoke, 26개 합성 corpus·계측·offset 평가 구현, 두 배치의 실제 Luna 26개 표본 일치; 의도 경계 보완 후 실모델 재검증·100개 확장·서사/장시간 평가 잔여 | 진행 중 |
...
### 원거리 전투 연결 (2026-09-21)

[단궁·화살·사선](ranged-combat.md)을 장비 구매와 전술 전투에 연결한다.
서버의 고정 벽과 정확한 선분 교차로 사선을 검증하고 화살·주요 행동을 함께 소비한다.
기존 근접 무기는 유지하며 활은 별도 슬롯과 공개 능력치를 사용한다.
벽 모서리, 최소/최대 거리, 인접 적, 탄약 부족, 빗나감, 저장·재전송 시 소비 보존을
검증한다. 일반 엄폐/시야, 다른 원거리 무기와 주문은 별도 잔여 범위다.

### 전투 회복력 연결 (2026-09-21)

현재 캐릭터는 직업이 지정되지 않았으므로 전체 클래스 구현을 가정하지 않는다.
[전투 회복력](combat-recovery.md)을 단일 자체 능력으로 추가해 보조 행동·현재 레벨·
휴식 비용·시간 기한·저장 복원을 연결한다. 서버가 충전과 회복량을 결정하고,
UI는 서버가 공개한 남은 횟수와 회복 공식을 표시한다. 능력 소진 뒤 새 전투나
패배 치료로 무료 재충전하지 않는 것을 필수 회귀로 둔다.

아래 게이트와 장시간 플레이 검증은 이 기능 추가 뒤에도 유효하다.
